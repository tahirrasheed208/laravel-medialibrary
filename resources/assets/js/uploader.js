...
$(document).on('change', '.ps-file-input', function () {
  ...
  $(this).next('.remove_img').removeClass('d-none');
});
...
$(document).on('click', '.htr-remove-file', function () {
  ...
  $(this).closest('.form-group').find('div.main-img-preview').addClass('d-none');
...
function readURL(input) {
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      ...
      $(input).parent().parent().prev('div').removeClass('d-none').find('.img-preview').attr('src', e.target.result);
    };
